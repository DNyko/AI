...
# Обчислимо коефіцієнт кореляції Пірсона для user1 і user2
def pearson_score(dataset, user1, user2):
    ...
    # Обчислення суми квадратів рейтингових оцінок усіх фільмів, оцінених обома користувачами
    user1_squared_sum = np.sum([np.square(dataset[user1][item]) for item in common_movies])
    user2_squared_sum = np.sum([np.square(dataset[user2][item]) for item in common_movies])

    # Обчислення суми товарів рейтингових оцінок усіх фільмів, оцінених обома користувачами
